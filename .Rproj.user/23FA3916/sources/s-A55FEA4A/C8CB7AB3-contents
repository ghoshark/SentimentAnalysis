...
ui <- dashboardPage(skin = "blue",
                    ...
                    dashboardSidebar(
                      ...
                      sidebarMenu(
                        ##Tab One
                        menuItem("Upload a File",tabName = "file",icon = icon("file-text-o")),
                        ##Tab Two
                        ...
                        ##Tab Nine
                        #menuItem("Barplot % y Word",tabName = "plotg",icon = icon("bar-chart-o")),
                        ##Tab Seven
                    ...
                    dashboardBody(
                      tabItems(
                        tabItem(tabName = "file",
                                fileInput("selection", "Upload an Excel File:",multiple = TRUE),
                                helpText(paste("Please upload a .xlsx file that ", 
                                         ...
                                         downloadButton("downloadromeo","Download Sample Text"),
                        ...
                        tabItem(tabName = "contact",
                                helpText(paste("Application Author: Arka Ghosh"),
                                         br(),
                                         br(),
                                         paste("Email: arkaghosh.nb@gmail.com"),
                                         br(),
                                         br(),
                                         paste("Phone: +91 9953156872"))),
...
server <- function(input, output, session) {
  ...
  ford <- reactive({ 
    req(input$selection) # require that the input is available
   ...
   survey <- readxl::read_xlsx("C:/Data_Science/SentimentAnalysis/raw_data.xlsx",sheet = "Survey Raw data", col_names = TRUE)
    ...
    # directions= RYandexTranslate::get_translation_direction(api_key)
    # head(directions)
    
#    Convert ALL Non-English strings to English
#    Call the mytranslate() function
#    for (i in 1:nrow(survey))
#    {
#      survey$Survey.Comment[i] <- mytranslate(api_key,survey$Survey.Comment[i],"en")[2]
#    }
    
    survey <- survey$Survey.Comment
    return(survey)
  ...
  getTermMatrix <- function(f) {
    ...
    docs<-tm_map(docs, removeWords,c(stopwords("SMART"),input$words))
    
    myDTM = TermDocumentMatrix(docs,control = list(minWordLength = 1,wordLengths=c(0,Inf)))
